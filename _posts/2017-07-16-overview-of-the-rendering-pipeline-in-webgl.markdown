...
example. The blue nodes are the shaders, these are microprograms we need to
write that are executed on the GPU for every vertex and pixel that needs to be
processed. Shader programs for WebGL are written in GLSL (OpenGL Shading
Language).
...
Now for the practical part, our application is split up into two functions, the
init function that's called when the page has finished loading and the render
function that's called at the end of the init function. The render function will
keep calling itself by calling requestAnimationFrame, with itself as argument.
...
As for the division of work between the two functions it's quite easy. The init
function takes care of the tasks that need to executed only once, such as
setting up the scene, compiling the shaders, etc... While the render function
takes care of the tasks that need to be performed every rendering cycle. And
it's here that the practical part will sometimes deviate from the conceptual
presentation. For our example it makes more sense to perform certain tasks once
instead of repeating them every rendering cycle.

## Setting up the scene

First we need to set up the scene. This involves loading our models and setting
up the camera as we need to choose a point of view from where we want to render
the scene. We also need to prepare our buffers that we'll use in our vertex
...
The viewport function of the WebGL context specifies the transformation from
...
lower left corner. We leave lower left corner coordinates to (0,0) and pass the
width and height of the HTML canvas element to the viewport function.

The clearColor function specifies the value that is used when clearing color
...
we also have to disable the depth test, as this will prevent the alpha blending
...
program, so we pass to the initShaders function the HTML id attributes to fetch
the shaders. The initShaders function takes care of the compiling and linking of
the shaders. We won't dive further in the code of this function but once again
you're free to download this project and take a look yourself.
...
getUniformLocation method to get an interface to the uniform variable in our
vertex shader. Using this interface we can pass the value of our view matrix to the shader using the uniformMatrix4fv function.
...
the proper value in w, the fourth element of the homogeneous coordinates, so
...
Clipping is the process that ensures that the vertices we're about to render are
 completely inside the view frustrum.
...
values. We define the aspect ratio by dividing the HTML canvas width by the
...
the near and far clip plane it specifies the shape of the view frustrum against
...
first by increasing the theta values every rendering cycle, subtracting
...
For the color we pass the colors we configured for the cube in two steps
...
rendering API so we do this in the init function.

We create an array which will hold the geometry we want to render. We add the
faces of the cube as triangles (polygons) and also the edges as line segments.
Now we need to set up our buffer and fill it with our data. First we create the
buffer using createBuffer(), we set it as the active buffer we want to work on
using the bindBuffer function, which binds our buffer to the ARRAY_BUFFER bind
point, a global variable internal to WebGL. Finally the bufferData function
...
attribute in the vertex shader, similar to how we created an interface to the
uniform variables, using the getAttribLocation function. Using the
 vertexAttribPointer function we specify the memory layout of the buffer
currently bound to the ARRAY_BUFFER bind point. In our example we specify that
our vertex attributes each hold four floating point numbers. And finally we
turn on the vertex attribute using the enableVertexAttribArray function.
...
geometry inside a geometry shader. We won't be discussing the geometry shader in
this article.
...
by a user-supplied microprogram called the vertex shader. The most important
...
We execute the shaders on the GPU by calling gl.drawArrays, specifying which
kind of primitives we want to draw, the starting index and the number of indices
we want to draw in that call. We do this every rendering cycle two times,
...
from degrees to radians, as this is the unit assumed by GLSL. Next we store
cosine and sine of the rotation angles in the c and n variables. Now we have
...
ry matrix defines a rotation around the y axis. Now we can apply our rotation
matrices, the model-view matrix and the projection matrix to the vertix
...
frustrum. In our example the primitive are lines and triangles. Primitives that
are completely outside the view frustrum are rejected as being invisible,
primitives that have one more vertices inside and one or more outside are
clipped so that all the vertices are inside the frustrum. Primitives that are
completely inside the frustrum are left as they were.
...
Next our 3D vertices are projected and mapped to the 2D screen-space coordinates
 ...
 level, such as colors, are interpolated across the face for each pixel. These
 values are then passed on the pixel shader.
...
This step is performed by a user-supplied microprogram called the pixel shader,
also referred to as the fragment shader. The shader is run once for every pixel.
...
shaders in a separate file. Normally the pixel shader would be where we could
...
line of the shader sets the precision that the GPU uses when calculating
floating points to medium, this ensures that our shader will run on all
...
pixel. If the depth test wasn't executed before the pixel shader than it's
executed now, if it is enabled. Next is the alpha value, a threshold is used
to test the opacity of the pixel, if it isn't high enough the pixel is rejected.
...
new value is combined with the existing one using the alpha value, a process
known as alpha blending.

This concludes our overview of the rendering pipeline. You can download the
project that we used as example with the download button below and take a more
in depth look at the code. Once again if you have questions or remarks feel
